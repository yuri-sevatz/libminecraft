...
namespace libminecraft
{
    namespace classic
    {
        ...
        inline void Client::read(server::Packet &dest)
        {
            ...
            if (!stream.good())
                throw NetworkException("Client connection closed while reading inbound minecraft packet");
        }
        ...
        inline void Client::write(const client::Packet &src)
        {
            ...
            if (!stream.good())
                throw NetworkException("Client connection closed while writing outbound minecraft packet");
        }
        ...
        inline MCTypes::Byte Client::next()
        {
            ...
            if (!stream.good())
                throw NetworkException("Client connection closed while awaiting next packet");
            return id;
        }
    }
}
